...
#' queried occurrences.
#' @param range_path (`character`) The path to read range of species for
#' spatial filtering.
...
filter_gbif <- function(
        x,
        occ_dir = here("data/occurrences_IUCN"),
    ...
    if(!file.exists(range_path)) stop("No range maps to use.")
    ...
    quality_flag <- data.frame(
        flag = c("Raw", "blank (gbif)", "age (gbif)", "precision (gbif)", 
                 ...
                 "institution (CC)", "insufficient", "range", "insufficient"),
        clean = NA, species = NA)
    ...
    # Step 1.3: Remove species with less than 4 records, 818 species removed
    ...
    # Save cleaned occ
    fname <- file.path(
        occ_dir, sprintf("occurrences_norange_%s", 
    ...
    catalog <- read.csv(fname) %>% 
        filter(speciesKey %in% unique(occ$speciesKey))
    ...
    ranges <- st_read(range_path) %>% 
        ...
        filter(sci_name %in% unique(catalog$sci_name))
    
    occ <- pbmclapply(unique(occ$species), function(sp){
        occ_tx <- occ %>% filter(species == sp)
        # Note that this could have multiple
        catalog_tx <- catalog %>% filter(species == sp)
        
        # Load AOH
        range_tx <- ranges %>% filter(sci_name %in% catalog_tx$sci_name) %>% 
            select(id_no) %>% st_simplify() %>% vect() %>% makeValid() %>% 
            buffer(width = 10000) %>% makeValid()
        ...
        pts <- occ_tx %>% select(decimalLatitude, decimalLongitude, gbifID) %>% 
            st_as_sf(coords = c("decimalLongitude", "decimalLatitude"),
                     crs = 4326) %>% vect()
        pts <- mask(pts, range_tx)
        
        occ_tx %>% filter(gbifID %in% pts$gbifID)
    ...
    # Save cleaned occ
    fname <- file.path(
        occ_dir, sprintf("occurrences_afterrange_%s", 
                         gsub(".zip", ".csv", basename(x))))
    write.csv(occ, fname, row.names = FALSE)
