...
prepare_vars <- function(clim_dir,
    ...
    dst_dir <- "results/experiments/10km_var_br_lc"
    env_dir <- file.path(dst_dir, "AllEnv")
    other_dir <- file.path(dst_dir, "OtherEnv")
    for (dr in c(env_dir, other_dir)){
    ...
    bry <- bry %>% st_transform(crs(template))
    ...
    vars <- c(sprintf("bio%s", c(1, 2, 12, 15)), "gst", "gsp")
    ...
    for (yr in years){
        ...
        if (yr == "1981-2010"){
            ...
            lc <- project(lc, template, method = "mode", threads = TRUE)
            ...
            for (mod in mods){
                for (ssp in ssps){
                    ...
                    clim <- rast(file.path(data_dir))
                    names(clim) <- vars
                    clim <- project(clim, template, threads = TRUE)
                    ...
                    area <- !is.na(lc[[1]])
                    area <- project(area, template, method = "sum")
                    
                    lc <- project(lc, template, method = "sum")
                    lc <- lc / area
