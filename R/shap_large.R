...
#' based on scenarios. A customized version for species with large range
...
#' shap_large(sp, work_dir, var_dir, range_dir)
#' }
#' 

shap_large <- function(sp,
                       work_dir = "results/sdm",
                       var_dir = "data/variables",
                       range_dir = "data/IUCN/Expert_Maps",
    ...
    if (!file.exists(fn)){
        ...
        len <- ceiling(nrow(new_data) / ncores)
        inds <- split(1:nrow(new_data), ceiling(1:nrow(new_data) / len))
        
        # The real calculation
        shap_explain <- mclapply(inds, function(ind){
            set.seed(seed, "L'Ecuyer-CMRG")
            
            fastshap::explain(
                mod, X = training,
                newdata = new_data[ind, ],
                nsim = nshap,
                adjust = TRUE, 
                pred_wrapper = pfun) %>% 
                as.data.frame()
